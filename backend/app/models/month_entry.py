...
class MonthEntry(Base):
    ...
    entry_mode: Mapped[EntryMode] = mapped_column(
        SqlEnum(EntryMode, values_callable=lambda x: [e.value for e in x]), nullable=False
    )
    status: Mapped[EntryStatus] = mapped_column(
        SqlEnum(EntryStatus, values_callable=lambda x: [e.value for e in x]),
    ...
    sofa_loan_entry_id: Mapped[int | None] = mapped_column(
        ForeignKey("sofa_loan_entries.id"), nullable=True
    )
